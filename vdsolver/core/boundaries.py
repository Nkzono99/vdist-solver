...
class PlaneXY(Plane2d):
    def __init__(self, z: float, func_prob: Callable[[np.ndarray], float]):
        super().__init__(2, z, func_prob)


class PlaneYZ(Plane2d):
    def __init__(self, x: float, func_prob: Callable[[np.ndarray], float]):
        super().__init__(0, x, func_prob)


class PlaneZX(Plane2d):
    def __init__(self, y: float, func_prob: Callable[[np.ndarray], float]):
        super().__init__(1, y, func_prob)
...
class RectangleX(ParallelRectangle):
    def __init__(self,
                 ...
                 dz: float,
                 func_prob: Callable[[np.ndarray], float]):
        super().__init__((0, 1, 2), pos, dy, dz, func_prob)
...
class RectangleY(ParallelRectangle):
    def __init__(self,
                 ...
                 dx: float,
                 func_prob: Callable[[np.ndarray], float]):
        super().__init__((1, 2, 0), pos, dz, dx, func_prob)
...
class RectangleZ(ParallelRectangle):
    def __init__(self,
                 ...
                 dy: float,
                 func_prob: Callable[[np.ndarray], float]):
        super().__init__((2, 0, 1), pos, dx, dy, func_prob)
...
def create_simbox(xlim: Tuple[float, float],
    ...
    if 'xl' in use_wall:
        xl = PlaneXY(xlim[0], fpdict['xl'])
    if 'xu' in use_wall:
        xu = PlaneXY(xlim[1], fpdict['xu'])
    if 'yl' in use_wall:
        yl = PlaneXY(ylim[0], fpdict['yl'])
    if 'yu' in use_wall:
        yu = PlaneXY(ylim[1], fpdict['yu'])
    if 'zl' in use_wall:
        zl = PlaneXY(zlim[0], fpdict['zl'])
    if 'zu' in use_wall:
        zu = PlaneXY(zlim[1], fpdict['zu'])
...
def create_box(xlim: Tuple[float, float],
    ...
    if 'xl' in use_wall:
        ...
        xl = RectangleX(org, dx, dy, fpdict['xl'])
    if 'xu' in use_wall:
        org = np.array([xlim[1], ylim[0], zlim[0]])
        xu = RectangleX(org, dx, dy, fpdict['xu'])
    if 'yl' in use_wall:
        org = np.array([xlim[0], ylim[0], zlim[0]])
        yl = RectangleY(org, dz, dx, fpdict['yl'])
    if 'yu' in use_wall:
        org = np.array([xlim[0], ylim[1], zlim[0]])
        yu = RectangleY(org, dz, dx, fpdict['yu'])
    if 'zl' in use_wall:
        org = np.array([xlim[0], ylim[0], zlim[0]])
        zl = RectangleZ(org, dx, dy, fpdict['zl'])
    if 'zu' in use_wall:
        org = np.array([xlim[0], ylim[0], zlim[1]])
        zu = RectangleZ(org, dx, dy, fpdict['zu'])
